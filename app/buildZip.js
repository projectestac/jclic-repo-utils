      ...
      project.files.forEach(function (file) {
        handlers.push(new Promise(function (resolve, reject) {
          ...
          if (avoidDir && file === 'project.json') {
            ...
            var prj = JSON.parse(JSON.stringify(project).replace(avoidDir, ''));
            ...
            if (!prj.files.includes('index.html')) {
              zip.file('index.html',
                indexTemplate
                  .replace(/%%TITLE%%/g, xmlStr(prj.title))
                  .replace(/%%MAINFILE%%/g, prj.mainFile));
              prj.files.push('index.html');

              if (!prj.files.includes('favicon.ico')) {
                zip.file('favicon.ico', Uint8Array.from(atob(favicon), function (c) { return c.charCodeAt(0); }), { binary: true });
                prj.files.push('favicon.ico');
              }

              if (!prj.files.includes('icon-72.png')) {
                zip.file('icon-72.png', Uint8Array.from(atob(icon72), function (c) { return c.charCodeAt(0); }), { binary: true });
                prj.files.push('icon-72.png');
              }

              if (!prj.files.includes('icon-192.png')) {
                zip.file('icon-192.png', Uint8Array.from(atob(icon192), function (c) { return c.charCodeAt(0); }), { binary: true });
            ...
            if (!prj.files.includes('imsmanifest.xml')) {
              ...
              zip.file('imsmanifest.xml',
                imsmanifestTemplate
                  ...
                  .replace(/%%TITLE%%/g, xmlStr(prj.title))
                  .replace(/%%FILES%%/g, prj.files.map(function (v) { return '<file href="' + xmlStr(v) + '"/>'; }).join('\n')));
            }

            zip.file(file.replace(avoidDir, ''), JSON.stringify(prj, null, ' '));
            resolve(zip);
          }
          else
...
<manifest identifier="JClic-%%ID%%" version="1.0" xmlns="http://www.imsproject.org/xsd/imscp_rootv1p1p2"\n\
 ...
 <resources>\n\
  ...
  %%FILES%%\n\
    ...
    var favicon = 'AAABAAEAEBAQAAAAAAAoAQAAFgAAACgAAAAQAAAAIAAAAAEABAAAAAAAwAAAAAAAAAAAAAAAAAAA\
...
AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA';
    ...
    var icon72 = 'iVBORw0KGgoAAAANSUhEUgAAAEgAAABICAYAAABV7bNHAAAABmJLR0QA/wD/AP+gvaeTAAAACXBI\
...
3ADLRsaTpY4pn7Z5bv8PFM+u32cIXhAAAAAASUVORK5CYII=';
    ...
    var icon192 = 'iVBORw0KGgoAAAANSUhEUgAAAMAAAADACAYAAABS3GwHAAAABHNCSVQICAgIfAhkiAAAAAlwSFlz\
